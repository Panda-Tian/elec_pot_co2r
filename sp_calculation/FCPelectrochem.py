...
class FCP(FileIOCalculator):
    ...
    def calculate(self, atoms=None,properties=('energy', ),
        ...
        ## modified by xwtian, avoid delete the file if it has already been exist, for continuing cal
        if not os.path.exists(LogPath+ '/tmp-log-FCP.txt'):
            ...
            tmp_data=pd.read_csv(LogPath+'/tmp-log-FCP.txt',sep='\t')
            if (len(tmp_data['NELECT']))>0:
                tmp_flag = True
            else:
                tmp_flag = False
        # with open(LogPath+ '/tmp-log-FCP.txt', mode='w',encoding='utf-8') as f:
        ...
        ## modified by xwtian, to restart the calculate
        def get_lrcount(arr):
            lrcount = 0
            filtered_arr = [x for x in arr if x != 0]  # filter zero first
            if len(filtered_arr) < 2:
                return lrcount
            for i in range(1, len(filtered_arr)):
                if filtered_arr[i]*filtered_arr[i - 1] < 0:
                    lrcount += 1                
            return lrcount
        ## modified by xwtian, find the closest WAVECAR for new calculation
        ...
        ## modified by xwtian, to restart the calculate
        if tmp_flag == True:
            self.FCPloop = tmp_data['loop'].iloc[-1]
            lr=1.0      # learning rate 0<lr<=1
            lrcount=get_lrcount(tmp_data['conv(V)'])
            convold = tmp_data['conv(V)'].iloc[-1]
            Nelectold = tmp_data['NELECT'].iloc[-1]
            self.C = tmp_data['Cpersurf(e/V/A^2)'].iloc[-1] * np.linalg.norm(np.cross(self.atoms.cell[0],self.atoms.cell[1]))
        else:
            self.FCPloop=0
            lr=1.0      # learning rate 0<lr<=1
            lrcount=0
        ...
        while self.FCPloop < self.max_FCP_iter:
            ...
            if self.FCPloop == 1 and self.Nelect0 == self.Nelect:  ## modified by xwtian to save the PZC outcar
                os.system(f"mv {LogPath}/OUTCAR {LogPath}/OUTCAR_pzc")
            #     os.system(f"mv {LogPath}/WAVECAR {LogPath}/WAVECAR_pzc")     
            #     os.system(f"mv {LogPath}/LOCPOT {LogPath}/LOCPOT_pzc")  
            Cevalue(self.Nelect)
            if convold != None:
                if convold*conv < 0:
                    lrcount+=1
            ...
            if abs(conv)<self.FCPconv:
                break
        os.system(f"tail -n +2 {LogPath}/tmp-log-FCP.txt >> fcp_all_log.txt")  # add by xwtian for logging all fcp time during md simulation!
        ...
        dict(magmom=atomstmp.calc.results['magmom'], 
            ...
            Nelect=self.Nelect,   # modified by xwtian, update Nelect number
            ))
